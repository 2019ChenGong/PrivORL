...
class TasksAug(nn.Module):
    def __init__(
        ...
        # 5TOKEN_COND VERSION: Separate MLPs for condition tokens (not shared with input)
        ...
        # MLPs for input sequence (separate from condition MLPs)
    ...
    def forward(self, x, time, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        # 5TOKEN_COND VERSION: Process condition as 5 separate tokens with dedicated MLPs
        if x_condition is not None:
            # x_condition shape: [batch, transition_dim]
        ...
        # 5TOKEN_COND VERSION: Concatenate time (1 token) + condition (5 tokens) + stacked_inputs (5H tokens)
        # Total sequence length: 1 + 5 + 5H = 6 + 5H
        ...
        # 5TOKEN_COND VERSION: prefix_mask now covers 6 tokens (1 time + 5 condition)
        prefix_mask = torch.ones((batch_size, 6), dtype=torch.bool, device=x.device)
