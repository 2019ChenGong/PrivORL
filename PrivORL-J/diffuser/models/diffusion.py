...
class AugDiffusion(GaussianInvDiffusion):
    def __init__(self, *args, loss_discount=1.0, loss_type='l1', action_weight=10, **kwargs):
        super().__init__(*args, loss_discount=loss_discount, loss_type=loss_type, action_weight=action_weight, **kwargs)
        self.transition_dim = self.action_dim + 2 * self.observation_dim + 2  
        self.action_weight = action_weight
        loss_weights = self.get_loss_weights(loss_discount)
        self.loss_fn = Losses[loss_type](loss_weights)
    ...
    def compute_loss(self, *batch):
        ...
        x = trajectories  # Already [batch_size, horizon, transition_dim]
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
    ...
    def _losses(self, x_start, t, cond=None):
        ...
        if x_recon is None:
            print("[Info] Model returned None (skipped batch)")
            return torch.tensor(0.0, device=x_noisy.device), {}
    ...
    def _mean_variance(self, x, t, cond=None):
        ...
        if self.clip_denoised:
            ...
            raise RuntimeError("Denoised samples are not clipped.")
