...
class TasksAug_Unet(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.horizon = horizon
        self.transition_dim = 2 * (transition_dim - 1) + action_dim + 2
        ...
        # pdb.set_trace()
        in_channels = self.transition_dim + dim + dim  # x + time + cond
        ...
        self.decoder = nn.Sequential(
            nn.Conv1d(hidden_size, hidden_size, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv1d(hidden_size, self.transition_dim, kernel_size=3, padding=1),  # output matches transition_dim
        )

    def forward(self, x, time, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        """
        Args:
            x: (B, H, D) = (batch_size, horizon, transition_dim)
            time: (B,)
            x_condition: (B, cond_dim)
        Returns:
            out: (B, H, D)
        """
        B, H, D = x.shape
        assert D == self.transition_dim

        # Time embedding, repeated per time step
        t_embed = self.time_mlp(time).unsqueeze(1).repeat(1, H, 1)  # (B, H, dim)

        if x_condition.shape[0] != B:
            print("there x_condition.shape[0] != B")
            pad_size = B - x_condition.shape[0]
            pad_cond = torch.zeros((pad_size, x_condition.shape[1]), device=x_condition.device, dtype=x_condition.dtype)
            x_condition = torch.cat([x_condition, pad_cond], dim=0)

        # Condition embedding: only injected at t=0
        if x_condition is not None:
            cond_embed = self.cond_mlp(x_condition)  # (B, dim)
            cond_seq = torch.zeros((B, H, self.dim), device=x.device)  # (B, H, dim)
            cond_seq[:, 0] = cond_embed  # only at first time step
        else:
            cond_seq = torch.zeros((B, H, self.dim), device=x.device)

        # Concatenate along feature dimension (dim=2)
        x_all = torch.cat([x, t_embed, cond_seq], dim=-1)  # (B, H, D + 2*dim)

        # U-Net expects (B, C, H), so permute
        x_unet = x_all.permute(0, 2, 1)  # (B, C, H)

        # U-Net path
        enc = self.encoder(x_unet)      # (B, hidden_size, H)
        mid = self.middle(enc)          # (B, hidden_size, H)
        dec = self.decoder(mid)         # (B, D, H)

        out = dec.permute(0, 2, 1)      # (B, H, D), same as input shape
        return out
