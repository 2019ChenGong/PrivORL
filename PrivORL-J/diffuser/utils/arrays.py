...
def batch_to_device(batch, device):
    """
    将batch中的所有tensor移动到指定设备
    """
    if hasattr(batch, 'trajectories'):
        batch.trajectories = batch.trajectories.to(device)
    if hasattr(batch, 'task'):
        batch.task = batch.task.to(device)
    if hasattr(batch, 'cond') and batch.cond is not None:
        batch.cond = batch.cond.to(device)
    if hasattr(batch, 'path_length') and batch.path_length is not None:
        batch.path_length = batch.path_length.to(device)
    if hasattr(batch, 'trajectory_ids') and batch.trajectory_ids is not None:
        batch.trajectory_ids = batch.trajectory_ids.to(device)
    if hasattr(batch, 'fragment_indices') and batch.fragment_indices is not None:
        batch.fragment_indices = batch.fragment_indices.to(device)
    return batch
