...
def watch(args_to_watch):
    def _fn(args):
        ...
        exp_name.append('-'+datetime.now().strftime('%b%d_%H-%M-%S'))
...
class Parser(Tap):
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'exp_name' in dir(args):
            if args.finetune:
                args.exp_name = f"finetune/epsilon{args.target_epsilon}_horizon{args.horizon}_curiosity{args.curiosity_rate}_{args.accountant}"
            else:
                args.exp_name = f"pretrain/horizon{args.horizon}_curiosity{args.curiosity_rate}"
