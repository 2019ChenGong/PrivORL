...
class MetaSequenceDataset(torch.utils.data.Dataset):
    def __init__(self, replay_dir_list=[], task_list=[], horizon=64,
        ...
        if use_d4rl:
            print("[INFO] Loading dataset from D4RL...")
            assert len(task_list) > 0, "D4RL requires at least one task name."
            env = gym.make(task_list[0])
            dataset = d4rl.qlearning_dataset(env)
            itr = self.parse_d4rl_dataset(dataset)
        else:
    ...
    def parse_d4rl_dataset(self, dataset):
        ...
        # 如果 terminals 全为 0，则按 max_path_length 切分
    ...
    def make_indices(self, path_lengths, horizon):
        ...
        for i, path_length in enumerate(path_lengths):
            ...
            if not self.use_padding:
                max_start = min(max_start, path_length - horizon - 1)
            for start in range(max_start):
                end = start + horizon
                indices.append((i, start, end))
...
class AugDataset(MetaSequenceDataset):
    ...
    def __getitem__(self, idx, eps=1e-4):
        ...
        rewards = self.fields.normed_rewards[path_inds, start:end].reshape(len(path_inds), end - start, 1)
        terminals = self.fields.normed_terminals[path_inds, start:end].reshape(len(path_inds), end - start, 1)

        # Handle next observations with padding if necessary
        next_observations = self.fields.normed_observations[path_inds, start + 1:end + 1]
        if next_observations.shape[1] < (end - start):
            padding = np.repeat(next_observations[:, -1:, :], (end - start) - next_observations.shape[1], axis=1)
            next_observations = np.concatenate([next_observations, padding], axis=1)
